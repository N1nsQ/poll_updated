...
function Create({ addNewPoll, updateQuestions }) {
  ...
  const validate = () => {
    ...
    temp.option = options.every(option => option.option.trim() !== "") ? "" : "All options are required.";
    setErrors(temp);
                ...
                <form noValidate autoComplete="off" onSubmit={handleSave}>
                  ...
                  {options.map((option, index) => (
                    <TextField
                      ...
                      variant="outlined"
                      {...(errors.option && { error: true, helperText: errors.option })}
                    />
                  ))}
